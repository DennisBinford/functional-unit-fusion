...
// Power-control baseline. Only the selected leaf receives live operands. The
// isolation muxes are part of the implementation and therefore part of PPA.
module demo_alu_separate_locked_isolated #(
  ...
  (* keep_hierarchy = 1 *) demo_op_add #(.WIDTH(WIDTH)) u_add
    (.a_i(op_i == 3'd0 ? a_i : '0), .b_i(op_i == 3'd0 ? b_i : '0), .result_o(add_result));
  (* keep_hierarchy = 1 *) demo_op_sub #(.WIDTH(WIDTH)) u_sub
    (.a_i(op_i == 3'd1 ? a_i : '0), .b_i(op_i == 3'd1 ? b_i : '0), .result_o(sub_result));
  (* keep_hierarchy = 1 *) demo_op_and #(.WIDTH(WIDTH)) u_and
    (.a_i(op_i == 3'd2 ? a_i : '0), .b_i(op_i == 3'd2 ? b_i : '0), .result_o(and_result));
  (* keep_hierarchy = 1 *) demo_op_or #(.WIDTH(WIDTH)) u_or
    (.a_i(op_i == 3'd3 ? a_i : '0), .b_i(op_i == 3'd3 ? b_i : '0), .result_o(or_result));
  (* keep_hierarchy = 1 *) demo_op_xor #(.WIDTH(WIDTH)) u_xor
    (.a_i(op_i == 3'd4 ? a_i : '0), .b_i(op_i == 3'd4 ? b_i : '0), .result_o(xor_result));
  (* keep_hierarchy = 1 *) demo_op_slt #(.WIDTH(WIDTH)) u_slt
    (.a_i(op_i == 3'd5 ? a_i : '0), .b_i(op_i == 3'd5 ? b_i : '0), .result_o(slt_result));
  (* keep_hierarchy = 1 *) demo_op_sll #(.WIDTH(WIDTH)) u_sll
    (.a_i(op_i == 3'd6 ? a_i : '0),
     .shamt_i(op_i == 3'd6 ? b_i[SHAMT_WIDTH-1:0] : '0),
     .result_o(sll_result));
